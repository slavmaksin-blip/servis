...
# Sentinel text the admin sends to finish multi-file upload
UPLOAD_DONE_TEXT = "✅ Готово"
...
async def admin_ban_execute(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    if not user_id:
        await message.answer(
            f"❌ Пользователь не найден: {target}", reply_markup=cancel_keyboard()
        )
        return
...
async def admin_add_product_name(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            "❌ Введите число (ID категории):", reply_markup=cancel_keyboard()
        )
        return
...
async def admin_add_product_save(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        f"Загрузите файлы товара через «📎 Загрузить файлы в товар».\n"
        f"Каждый загруженный файл = 1 единица товара в наличии.",
        reply_markup=admin_keyboard(),
        parse_mode="HTML",
    )


# ─── UPLOAD FILES (STOCK MANAGEMENT) ─────────────────────────────────────────────
# Admin sends multiple files; each file = 1 stock unit added to the product.
# Send UPLOAD_DONE_TEXT ("✅ Готово") to finish the upload session.

@router.callback_query(F.data == "admin:upload_files")
async def admin_upload_files_product(callback: CallbackQuery, state: FSMContext) -> None:
    if not _is_admin(callback.from_user.id):
        return
    await callback.message.answer(
        "📎 Введите ID товара для загрузки файлов:",
        reply_markup=cancel_keyboard(),
    )
...
async def admin_upload_files_ask_file(message: Message, state: FSMContext) -> None:
    if not _is_admin(message.from_user.id):
        return
    if message.text == "❌ Отмена":
        await _admin_cancel(message, state)
    ...
    try:
        ...
        await message.answer(
            "❌ Введите число (ID товара):", reply_markup=cancel_keyboard()
        )
        return
    ...
    if not product:
        await message.answer(
            f"❌ Товар с ID {product_id} не найден.", reply_markup=cancel_keyboard()
        )
        return

    current_stock = await get_product_stock(product_id)
    await state.update_data(upload_product_id=product_id, upload_count=0)
    await message.answer(
        f"📎 <b>Загрузка файлов для товара «{product['name']}» (ID: {product_id})</b>\n\n"
        f"Текущий остаток: {current_stock} шт.\n\n"
        f"Отправляйте файлы по одному. Каждый файл добавит 1 единицу в наличие.\n"
        f"Когда закончите, отправьте: <b>{UPLOAD_DONE_TEXT}</b>",
        reply_markup=cancel_keyboard(),
        parse_mode="HTML",
    )
...
async def admin_upload_file_receive(message: Message, state: FSMContext) -> None:
    if not _is_admin(message.from_user.id):
        return
    ...
    if message.text == UPLOAD_DONE_TEXT:
        data = await state.get_data()
        product_id = data.get("upload_product_id")
        count = data.get("upload_count", 0)
        total_stock = await get_product_stock(product_id)
        await state.clear()
        await message.answer(
            f"✅ <b>Загрузка завершена!</b>\n\n"
            f"Добавлено файлов за сессию: <b>{count}</b>\n"
            f"Всего в наличии: <b>{total_stock}</b> шт.",
            reply_markup=admin_keyboard(),
            parse_mode="HTML",
        )
        return

    # Extract file_id from any supported media type
    ...
    if not file_id:
        await message.answer(
            f"❌ Отправьте файл (документ, фото, аудио или видео) "
            f"или «{UPLOAD_DONE_TEXT}» для завершения."
        )
        return

    data = await state.get_data()
    product_id = data.get("upload_product_id")
    count = data.get("upload_count", 0) + 1
    await state.update_data(upload_count=count)

    await add_product_file(product_id, file_id)
    total_stock = await get_product_stock(product_id)

    await message.answer(
        f"✅ Файл #{count} добавлен. Всего в наличии: <b>{total_stock}</b> шт.\n"
        f"Отправьте следующий файл или «{UPLOAD_DONE_TEXT}» для завершения.",
        parse_mode="HTML",
    )
...
async def admin_give_balance_amount(
...
async def admin_give_balance_execute(
    message: Message, state: FSMContext, bot: Bot
) -> None:
    if not _is_admin(message.from_user.id):
        return
    ...
    try:
        ...
        await message.answer(
            "❌ Введите корректную сумму:", reply_markup=cancel_keyboard()
        )
        return
    data = await state.get_data()
...
async def admin_take_balance_amount(
    message: Message, state: FSMContext, bot: Bot
) -> None:
    if not _is_admin(message.from_user.id):
        return
...
async def admin_balances(callback: CallbackQuery) -> None:
    ...
    for u in balances[:MAX_DISPLAYED_BALANCES]:
        name = (
            f"@{u['username']}"
            if u["username"]
            else u.get("first_name") or str(u["user_id"])
        )
...
async def admin_promo_code(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "🎁 Введите сумму в USD для начисления:", reply_markup=cancel_keyboard()
    )
...
async def admin_promo_limit(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            "❌ Введите корректное число:", reply_markup=cancel_keyboard()
        )
        return
    data = await state.get_data()
