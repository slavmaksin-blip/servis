    ...
    if os.path.exists(photo_path):
        ...
        try:
            ...
            await callback.message.answer(
                text, reply_markup=profile_keyboard(), parse_mode="HTML"
            )
    else:
        try:
...
async def promo_entered(message: Message, state: FSMContext) -> None:
    ...
    if not promo:
        await message.answer("❌ Промокод не найден.", reply_markup=cancel_keyboard())
        return

    user_id = message.from_user.id
    if await has_used_promo(promo["id"], user_id):
        await message.answer(
            "❌ Вы уже использовали этот промокод.", reply_markup=cancel_keyboard()
        )
        return

    if promo["used_count"] >= promo["limit_uses"]:
        await message.answer(
            "❌ Лимит активаций промокода исчерпан.", reply_markup=cancel_keyboard()
        )
        return
