...
from app.services.de_prank_pdf import generate_de_prank_bank_pdf
...
async def pdf_input_holder(message: Message, state: FSMContext) -> None:
    ...
    iban_hint = "DE** **** **** **** **** **XXXX" if data.get("country") == "de" \
        else "CH** **** **** **** **XXXX"
    await message.answer(
        "<b>Шаг 2/4 — Последние 4 цифры IBAN:</b>\n"
        f"Введи последние 4 цифры IBAN (например: <i>7809</i>).\n"
        f"В документе IBAN будет отображён как <code>{iban_hint}</code>.",
        parse_mode="HTML",
    )
...
async def pdf_input_service(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"<b>Шаг 4/4 — Сумма в {currency}:</b>\n"
...
async def pdf_input_amount(message: Message, state: FSMContext) -> None:
    ...
    if country == "de":
        pdf_bytes = generate_de_prank_bank_pdf(
            account_holder=account_holder,
            iban_suffix=iban_suffix,
            service_name=service_name,
            amount_eur=amount,
        )
        filename_prefix = "kontoauszug_de"
    else:
    ...
    filename = f"{filename_prefix}_{safe_name}.pdf"
