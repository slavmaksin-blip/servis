...
def email_domains_keyboard(domains: list[dict]) -> InlineKeyboardMarkup:
    ...
    builder.adjust(1)
    builder.row(
        InlineKeyboardButton(text="❌ Отмена", callback_data="shop:email_cancel")
    )
    return builder.as_markup()
...
def email_actions_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="📨 Получить сообщение", callback_data="email:get_message"
        )
    )
    builder.row(
...
def categories_keyboard(categories: list[dict]) -> InlineKeyboardMarkup:
    ...
    builder.adjust(2)
    builder.row(
        InlineKeyboardButton(text="❌ Отмена", callback_data="shop:products_cancel")
    )
    return builder.as_markup()
...
def products_keyboard(products: list[dict]) -> InlineKeyboardMarkup:
    """
    Build product buttons showing current stock count.
    Button label format: "Name [Stock]  — $Price"
    """
    builder = InlineKeyboardBuilder()
    for p in products:
        stock = p.get("stock", 0)
        builder.add(
            InlineKeyboardButton(
                text=f"{p['name']} [{stock}] — ${p['price']:.2f}",
                callback_data=f"prod:{p['id']}",
            )
        )
    builder.adjust(1)
    builder.row(
        InlineKeyboardButton(text="⬅️ Назад", callback_data="shop:products")
    )
    return builder.as_markup()
...
def product_confirm_keyboard(product_id: int, quantity: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="✅ Подтвердить",
            callback_data=f"buy_confirm:{product_id}:{quantity}",
        )
    )
    builder.row(
...
def admin_categories_keyboard(categories: list[dict]) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="➕ Добавить категорию", callback_data="admin:add_category"
        )
    )
    builder.row(
...
def admin_products_keyboard(products: list[dict]) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="📎 Загрузить файлы в товар", callback_data="admin:upload_files"
        )
    )
    builder.row(
        InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:back")
